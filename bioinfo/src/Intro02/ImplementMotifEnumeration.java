...
public class ImplementMotifEnumeration {
    ...
    public static void ImplementMotifEnumeration() throws FileNotFoundException {
        ...
        BiFunction<String, String, Boolean> isDNeighbor = (pattern1, pattern2) -> {
            return d >= (int) IntStream.range(0, pattern1.length())
                    .parallel()
        ...
        Set<String> motifs = neighborsToCount.keySet()
                .stream()
                .filter(neighbor -> {
                    return patterns.size() == patterns.stream().filter(line -> {
                        return line.stream().anyMatch(p -> {
